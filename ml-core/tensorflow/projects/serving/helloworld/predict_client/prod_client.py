...
class ProdClient:
  ...
  def predict(self, request_data, request_timeout=10):
    ...
    # The 'map<string, TensorProto> inputs = 2;' in predict.proto corresponds to
    # the parameter inputs that we send to the signature definition in our model.
    # Here we fill in the inputs. These protos are copied from tensorflow serving
    # apis:
    #   https://github.com/tensorflow/serving/tree/r1.7/tensorflow_serving/apis
    t = time.time()
    ...
    try:
      # In the bottom of predict.proto, PredictResponse is defined. This message
      # has one field: 'map<string, TensorProto> outputs = 1;'. The key in this
      # map is our ‘add’ from our model.
      t = time.time()
