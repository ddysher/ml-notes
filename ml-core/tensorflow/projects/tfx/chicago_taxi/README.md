...
> Copied from https://github.com/tensorflow/model-analysis/tree/v0.11.0/examples/chicago_taxi
...
> The goal is to predict whether a passenger is a big tipper, by testing if
> the tip was > 20% of the fare.
...
> The `preprocessing_fn` written by user contains the logic for transforming
> dataset. It accepts and returns a dictionary of tensors. TensorFlow Transform
> will construct a graph and run the graph (i.e. session.run) to realize the
> output from the preprocessing function. It then saves the graph to a `SavedModel`
> and creates `TFRecord` files.
...
> The two models are similar but with different purpose:
> - Serving model receives input and returns prediction
> - Evaluation model receives input (sepcifically, full pass of dataset) and returns evaluation metrics
>
> The two models are exported in different methods:
> - The serving `SavedModel` is exported via `tf.estimator.FinalExporter`
> - The evaluation `SavedModel` is exported via `tfma.export.export_eval_savedmodel`
>
> Both methods require a receiver function, which returns a graph that reads
> input value, parses it using the same schema, and applies the same
> transformation.
