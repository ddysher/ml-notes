...
void vm_init(struct vm *vm, size_t mem_size)
{
  ...
  // Uses mmap to allocate some memory for the VM. Physical memory of the
  // VM corresponds to user space memory of current process.
...
void vcpu_init(struct vm *vm, struct vcpu *vcpu)
{
  ...
  // Makes a KVM_CREATE_VCPU call to creates a VCPU within the VM.
  ...
  // mmaps vcpu control area; we can read/write the memory to access
  // and control vcpu.
...
int run_long_mode(struct vm *vm, struct vcpu *vcpu)
{
  ...
  //
  // Setup sregs, e.g. cs, ds, gdt, cr3, etc, see "/arch/x86/include/uapi/asm/kvm.h"
  //
  if (ioctl(vcpu->fd, KVM_GET_SREGS, &sregs) < 0) {
    perror("KVM_GET_SREGS");
    exit(1);
  ...
  //
  // Setup regs, e.g. rax, rsi, r8, r9, rflags, etc, see "/arch/x86/include/uapi/asm/kvm.h"
  //
  memset(&regs, 0, sizeof(regs));
...
int main(int argc, char **argv)
{
  ...
  // Create VM with 2G memory.
  vm_init(&vm, 0x200000);

  // Create vcpu. Here we just call kvm API interface, in
  // qemu, each vcpu has a thread to manage it.
  vcpu_init(&vm, &vcpu);
